...
def _parse_time_range(time_str):
    ...
    # Split on '-' but handle negative: find the dash between two time parts
    # Pattern: detect start-end where each part is like: \d+(:\d+)?(am|pm)?
    # We split on the '-' that separates two time parts
    # Use regex to find the split point
